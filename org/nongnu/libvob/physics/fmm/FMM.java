...
FMM.java
 *    
 *    Copyright (c) 2000, Kimmo Wideroos
 *                  2004, Matti J. Katila
 *
 ...
 *    copied to libvob from gzz sources 2004 by Matti J. Katila
 */
...
public class FMM {
	...
	Particle[] getParticles(FMMCell fc);
	Particle getParticle(float x, float y);
	int nLevel();
	int nTerms();
	void empty();
	Particle makeParticle(float x, float y, int[] q);
	Particle add(Particle fp);
     ...
     * having weight. 
     */
    public int proceed(int qind, float weight, boolean augm) {
	this.qind = qind;
	this.weight = weight;
	this.augm = augm;
	...
	for(ibox=0; ibox<fcs.length; ibox++) {
	    fc = fcs[ibox];
	    Particle[] fps = fmms.getParticles(fc);
	...
	*/
	...
	// step 5
	for(ibox=0; ibox<fcs.length; ibox++) {
	    Particle[] fps = fmms.getParticles(fcs[ibox]);
	    for(int i=0; i<fps.length; i++)
    ...
    */
    ...
    public void particle_interactions(Particle fp, FMMCell fc) {
	...
	for(int ibox=0; ibox<fcs.length; ibox++) {
	    ...
	    Particle[] nbparticles = fmms.getParticles(nbc);
	    for(int i=0; i<nbparticles.length; i++) {
		Particle nbp = nbparticles[i];
    ...
    public void compute_fields(Particle fp, FMMCell fc) {
