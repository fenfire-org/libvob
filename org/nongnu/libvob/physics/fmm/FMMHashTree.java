...
package org.nongnu.libvob.physics.fmm;
import org.nongnu.libvob.physics.Particle;
import org.nongnu.libvob.util.math.Complex;
import java.util.*;
...
public class FMMHashTree implements FMM.FMMStructure {
    ...
    public Particle add(Particle fp) {
	int key;
    ...
    public Particle makeParticle(float x, float y, int q) {
	//return new Particle(x, y, new int[] {q}, p);
	return null;
    }

    public Particle makeParticle(float x, float y, int[] q) {
	//return new Particle(x, y, q, p);
	return null;
    }
     ...
     */
    private void addParticle(FMMCell fc, Particle fp) {
	if(!fmmparticles.containsKey(fc)) 
	    fmmparticles.put(fc, new Particle[] {fp});
	else {
	    Particle[] curpl = (Particle[])fmmparticles.get(fc);
	    Particle[] newpl = new Particle[curpl.length+1];
	...
	Particle[] fps = (Particle[])fmmparticles.get(fc);
    ...
    public Particle[] getParticles(FMMCell fc) {
	if(!fmmparticles.containsKey(fc)) {
	    return new Particle[] {};
	}
	return (Particle[])fmmparticles.get(fc);
    }
   ...
   public Particle[] getAllParticles() {
	   ...
	   fps0 = (Particle[])e.nextElement();
	   for(i=0; i<fps0.length; i++) {
	       fpv.add(fps0[i]);
	   }
       }
       fps = new Particle[fpv.size()];
       for(i=fps.length-1; i>=0; i--) {
	   fps[i] = (Particle)fpv.elementAt(i);
       }
       return fps;
   }
