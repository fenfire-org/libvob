...
def load(file):
    ...
    for (k,v) in vob.util.faildefs.__dict__.items():
        if k.startswith('fail'):
            module.__dict__[k] = v
    module.needGL = needGL
    ...
    class Starter(java.lang.Runnable):
        ...
        def run(self):
            while 1:
                global loop
                try:
                    exceptions = self.runOne()
                except SyntaxError: print 'syntax error'

                if not loop: break
                print "\n\n"+'Tests have finished. Press Enter to retest.'+"\n"+ \
                      'Enter any other key to exit the loop and quit.'
                if  raw_input().lower() != '': break
            java.lang.System.exit(exceptions != [])

        def runOne(self):
            # update manager must have been started first
            global gfx
            from test.tools import gfx as gfx0
            gfx = gfx0
